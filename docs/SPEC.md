...
Fetch events from nostr relays in real time, the events we are fetching are authored by the user (pubkey field == user's pubkey), kind 31925, only display on the home screen the ones with creation time less than 24 hours. Only show by default the events in waiting and in-progress status, with a filter where the user can filter by other statuses.
...
- **Kind**: 31925 (addressable event → NIP-1)
...
- Show events kind 31925 with status `waiting`, `in-progress` (default filter)
...
- Publishes kind 31925 event with status: `waiting`
